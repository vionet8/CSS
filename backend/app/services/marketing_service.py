...
# キャッチコピー生成の切り口（1案につき1テーマ）
CATCHCOPY_THEMES = [
    "ベネフィット直球（得られる未来をそのまま言う）",
    "数字・実績（具体的な数値で信頼を作る）",
    "問いかけ（読者の悩みを質問にする）",
    "損失回避（やらないと何を失うかを示す）",
    "ターゲット呼びかけ（「〜なあなたへ」と名指しする）",
    "意外性・逆説（常識の逆を突いて目を止める）",
    "比喩・たとえ（別のものに例えて直感的に伝える）",
    "社会的証明（利用者・支持の広がりを示す）",
    "緊急性・限定（今やる理由を作る）",
    "情緒・共感（気持ちに寄り添う）",
]
...
ASSET_TYPES: dict[str, dict] = {
    "catchcopy": {
        "label": "キャッチコピー",
        "description": "切り口の異なる10案",
        "instruction": (
            "キャッチコピーを10案。1案につき下記のテーマを1つずつ順番に使い、"
            "title=テーマ名（例: 数字・実績）、text=キャッチコピー本体（30字以内）とする。\n"
            "テーマ一覧:\n"
            + "\n".join(f"{i+1}. {t}" for i, t in enumerate(CATCHCOPY_THEMES))
            + "\n素材に数値・実績・利用者の声が無い場合、「数字・実績」「社会的証明」は"
            "捏造せず、そのテーマの枠で使える別の表現（例: 開発の背景・機能の具体性）に置き換える。"
        ),
        "count": 10,
    },
    "x_post": {
